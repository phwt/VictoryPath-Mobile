...
export default function Easy(props) {
  ...
  if (gameplay) var timedisplay = 3000; //เวลาที่จะแสดงสีค้างไว้
  ...
  function addToAnswer(number) {
    if (activeColor == "#7F8080") {
      if (number == pathList[click]) {
        ...
        console.log("Defeat!!!");
        // <Modal isVisible={isModalVisible} onPress={() => addScore()}>
        //   <Text>Defeat!</Text>
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < size; j++) {
        if (pathList.includes(buttonValue[i][j])) {
          row.push(
            <PathButton onPress={() => addToAnswer(buttonValue[i][j])} />
          );
        } else {
          row.push(
